...
export const useVirtualRooms = () => {
  ...
  // Create a new virtual room - direct database insert
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          theme: roomData.theme || 'neon_city',
          room_config: roomData.room_config || {},
          background_color: '#1a0f2e',
          ambient_lighting: roomData.ambient_lighting || { intensity: 0.5, color: '#ffffff' }
        })
        .select()
        .single()
  ...
  // Get a specific room - direct query, works without auth for public rooms
  const getRoom = useCallback(async (roomId: string): Promise<VirtualRoom | null> => {
    ...
    try {
      ...
      const room = {
        ...
        ambient_lighting: data.ambient_lighting || { intensity: 0.5, color: '#ffffff' },
      }
  ...
  // Get public rooms - direct query without auth requirement
  const getPublicRooms = useCallback(async (): Promise<VirtualRoom[]> => {
    ...
    try {
      ...
      // Map data to match VirtualRoom interface
      const rooms = (data || []).map(room => ({
        ...room,
        is_featured: room.is_featured || false,
        ambient_lighting: room.ambient_lighting || { intensity: 0.5, color: '#ffffff' },
      }))
