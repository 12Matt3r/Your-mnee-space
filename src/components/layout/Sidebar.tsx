...
const publicNavigation = [
  ...
  { name: 'Artist Directory', href: '/artists', icon: UsersIcon },
...
const creatorNavigation = [
  { name: 'Go Live', href: '/live', icon: VideoCameraIcon, requiresAuth: true },
  ...
  { name: 'Wallet', href: '/wallet', icon: WalletIcon, requiresAuth: true },
...
export const Sidebar = () => {
    ...
    return (
      <Link
        ...
        className={cn(
          ...
          isActive && user
            ? 'bg-mnee-gold/20 text-mnee-gold'
            : isDisabled
            ? 'text-gray-500 cursor-not-allowed'
            : 'text-gray-300 hover:bg-mnee-gold/10 hover:text-mnee-gold'
        )}
      >
        <item.icon
          className={cn(
            ...
            isActive && user
              ...
              : isDisabled
              ? 'text-gray-500'
              : 'text-gray-400 group-hover:text-mnee-gold'
          )}
        />
        {item.name}
  ...
  return (
    <div className="w-64 h-screen bg-mnee-black/20 backdrop-blur-xl border-r border-mnee-slate flex flex-col">
      {/* Logo */}
      <div className="p-6 border-b border-mnee-slate">
        <Link to="/" className="flex items-center space-x-3">
          <div className="w-8 h-8 bg-gradient-to-r from-mnee-gold to-mnee-blue rounded-lg flex items-center justify-center">
            <span className="text-mnee-black font-bold text-lg">Y</span>
          </div>
          <span className="text-xl font-bold text-white">YourSpace</span>
        </Link>
      </div>
      ...
      {/* Bottom Navigation */}
      <div className="p-3 border-t border-mnee-slate space-y-2">
        {bottomNavigation.map(renderNavItem)}
      </div>

      {/* User Profile Summary or Auth Prompt */}
      <div className="p-4 border-t border-mnee-slate">
        {user && profile ? (
          // Authenticated user profile
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-r from-mnee-gold to-mnee-blue flex items-center justify-center">
              {profile.avatar_url ? (
                <img 
          ...
          <div className="text-center">
            <div className="w-12 h-12 mx-auto mb-3 bg-gradient-to-r from-mnee-gold to-mnee-blue rounded-full flex items-center justify-center">
              <UserIcon className="h-6 w-6 text-mnee-black" />
            </div>
            ...
            <div className="space-y-2">
              <Link
                to="/register"
                className="block w-full py-2 px-3 bg-gradient-to-r from-mnee-gold to-mnee-goldDark rounded-lg text-mnee-black text-sm font-medium hover:shadow-mnee-gold transition-all duration-200"
              >
                Sign Up
              </Link>
              <Link
                to="/login"
                className="block w-full py-2 px-3 border border-mnee-slate rounded-lg text-mnee-gold text-sm font-medium hover:bg-mnee-gold/10 transition-all duration-200"
              >
                Sign In
              </Link>
            </div>
