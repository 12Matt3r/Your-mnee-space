        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {displayItems.map((item) => (
            <Link
              key={item.id}
              to={`/content/${item.id}`}
              className="group relative aspect-square rounded-lg overflow-hidden border border-mnee-slate hover:border-mnee-gold/40 transition-all"
            >
              {item.file_url ? (
                <img
                  src={item.file_url}
                ...
                <div className="w-full h-full bg-gradient-to-br from-mnee-blue/20 to-mnee-black flex items-center justify-center">
                  <span className="text-mnee-gold text-xs uppercase font-bold tracking-wider">
                    {item.content_type}
                  </span>
                </div>
              )}
